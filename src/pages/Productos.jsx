      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 text-cafe-oscuro">
        <div className="flex items-center justify-between mb-12 pb-16 border-b-1 border-cafe-oscuro/30">
          ...
          {categories.length > 0 && (
            <div className="flex items-center gap-2">
              <label
                htmlFor="categoria"
                className="font-texto text-sm text-cafe-oscuro"
              >
                Categoría:
              </label>
              <div className="relative">
                <select
                  ...
                  className="font-texto text-sm border border-cafe-oscuro/30 rounded-xl px-4 py-2 bg-white/80 focus:outline-none focus:ring-2 focus:ring-cafe-oscuro/40 appearance-none pr-12"
                >
                  <option className="bg-cafe-oscuro text-cafe-claro" value="Todos">
                    Todos
                  </option>
                  {categories.map((cat) => (
                    <option className="bg-cafe-oscuro text-cafe-claro" key={cat} value={cat}>
                      {cat}
                    </option>
                  ))}
                </select>
                <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center p-4">
                  <svg
                    className="h-4 w-4 text-cafe-oscuro/70"
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {grupo.map((producto) => (
                      <div
                        key={producto.id}
                        className="bg-white/80 rounded-xl overflow-hidden flex flex-col border-1 border-cafe-oscuro"
                      >
                        <img
        ...
        {showScrollButton && (
          <button
            ...
            className="fixed bottom-4 right-4 bg-cafe-oscuro text-cafe-claro px-6 py-4 m-8 rounded-full shadow-lg hover:bg-cafe-oscuro/90 focus:outline-none hover:cursor-pointer"
